...
estimators, and that is the gap closed here. Two further captures with
the radio off, of $3.52\times10^6$ and $8.38\times10^6$ bits, were taken
during this work with the verifier firmware of
Chapter~\ref{ch:ch5_verification}: same silicon, different board, firmware
and day. All six were run through the
\ac{NIST} reference implementation of the SP 800-90B
estimators~\cite{nist_ea_github, turan_2018_sp80090b} (version 1.1.8),
on both the IID and the non-IID tracks, in the binary layout the standard
prescribes for bit sources. Table~\ref{tab:esp32_90b} gives the outcome.

\begin{table}[htbp]
\centering
\caption{The ESP32 generator under the SP 800-90B estimators \meas{}. $N$
is the number of bits; the ceiling is the highest value the most-common-value
(MCV) estimator can return for a perfectly uniform source of that length
at 99\,\% confidence; the non-IID figure is the minimum of the ten
estimators of Section~6.3 of the standard, and the last column names the
estimator that set it.}
\label{tab:esp32_90b}
\footnotesize
\begin{tabular}{@{}llrlrrrl@{}}
\toprule
Capture & Condition & $N$ (bits) & IID & $H$ (IID) & Ceiling & $H$ (non-IID) & Limiting \\
\midrule
A & Wi-Fi + ADC & $10^6$ & pass & 0.9940 & 0.9963 & 0.8255 & Compression \\
B & Wi-Fi       & $10^6$ & pass & 0.9949 & 0.9963 & 0.8980 & LRS \\
C & ADC         & $10^6$ & pass & 0.9960 & 0.9963 & 0.8199 & Compression \\
D & none        & $10^6$ & pass & 0.9959 & 0.9963 & 0.9155 & Collision \\
E (this work) & none & $3.5\times10^6$ & pass & 0.9977 & 0.9980 & 0.8671 & Compression \\
F (this work) & none & $8.4\times10^6$ & pass & 0.9982 & 0.9987 & 0.9108 & Compression \\
\bottomrule
\end{tabular}
\end{table}
...
0.9980 with $3.5\times10^6$ or 0.9987 with $8.4\times10^6$. Every capture
sits within 0.0023 of that ceiling, and none shows a bias of ones beyond
$1.6\sigma$. On the IID track, which all six pass (permutation tests and
$\chi^2$), the ESP32
output is therefore indistinguishable from a full-entropy binary source at
this sample size. That is the same verdict SP 800-22 gave, reached by a
different route, and it says the same thing: nothing wrong is visible.

The non-IID track says something else, and reading it correctly matters
more than the number. Its minimum is set in every case by one of three
estimators --- compression, collision or longest repeated substring --- at
0.82 to 0.92 bit per bit, while the MCV, Markov and the four predictors
never drop below 0.99. Those three estimators are known to be conservative
on nearly uniform data~\cite{zhu_2017_analysis}; the predictors of Kelsey, McKay and S\"onmez
Turan~\cite{kelsey_2015_predictive} were introduced precisely because the
earlier estimators of the standard misjudge sources of that kind. The
captures themselves show that this is estimator behaviour and not a
property of the conditions: three captures of the \emph{same} condition
(D, E and F) span 0.87 to 0.92 on the compression estimate, as much as the
whole spread between conditions. The largest capture adds a second
illustration of the same effect: in the byte layout its limiting
estimator is the $t$-tuple test at $t = 2$, which returns 7.39 bits per
byte because $10^6$ bytes hold too few of the 65\,536 possible pairs for
the most frequent one to be representative, exactly as the MCV ceiling
does at $t = 1$. With one repetition per condition, no
difference between Wi-Fi on, ADC on and neither is resolvable by any
estimator, and none is claimed. One anomaly is recorded without
interpretation: in the byte layout, capture D fails the $\chi^2$
...
and captures E and F, taken under the same condition with three and eight
times more data, pass it with $p = 0.27$ and $p = 0.28$. Twelve such
tests were run; one failure at that level has about a one-in-a-hundred
chance of being noise, and a second repetition of condition D is the only
thing that settles it.

Three consequences follow for O8. First, the figure this thesis carries
forward as the ESP32 reference is the non-IID one, 0.82 to 0.92 bit per
bit, because it is what an evaluator would credit without granting the
...
entropy costs 256 output bits in the first and up to 313 in the second,
...
as measured.
\item[Min-entropy of the ESP32 reference.] Measured \meas{} on
$10^6$ to $3.5\times10^6$ bits per capture. The IID-track figure is at the
resolution ceiling of the MCV estimator for that $N$ (0.9963 at $10^6$
bits), so it is a floor on the ceiling rather than a measurement of the
source; the non-IID figure is set by estimators whose conservative bias on
nearly uniform data exceeds the spread between conditions, and with one
repetition per condition no condition effect is claimed.
