...
Whole engine & ESP32 sequence over I2C: id, seed (divider below minimum refused, divider frozen while busy), key, random register free of the key, 2 challenges, 2nd key, capture, key hidden & 18/18 \\
\bottomrule
\end{tabular}
\end{table}
...
bench. The eighteen checks pass, and the key, the two challenges and their
...
Two of the eighteen checks were added when two obligations that the
register map had placed on the host were moved into the hardware: a seed
command issued with the divider below its minimum is now refused, and a
write to the divider register while a command is running is ignored. A
rule that only the documentation enforces is a rule that a careless host
breaks; the bench now exercises both. The bench also had to be told to
stop: with the rings running after reset, a simulation whose stimulus has
finished never runs out of events, and the first re-run consumed
forty-five minutes of CPU after its last check had passed.
